...
public class Call_Sp {
	...
	public Sub_Obj_SA[] obtenSumaA(){
		...
		try{
			...
			sp.registerOutParameter("OUT_C", OracleType.CURSOR);
			sp.execute();
			ResultSet rs =(ResultSet)sp.getObject("OUT_C");
			while(rs.next()){
				i =i+1;
			...
			i=0;
			while(rs.next()){
				m[i]=f.asignaSA(rs.getDouble("SA"));
				i=i+1;
			}
			con.close();
				}catch(SQLException e){
					m[0]=f.asignaSA("Error",e.getMessage());
					return m;
				}finally{
					return m;
	...
	public Sub_Obj_Deducible[] obtenDeducible(){
		int i =0;
		Func_auxiliar f= new Func_auxiliar();
		...
		try{
			...
			i=0;
			while(rs.next()){
				m[i]=f.asignaDeducible(rs.getDouble("deducible"));
				i=i+1;
			}
			con.close();
				}catch(SQLException e){
					m[0]=f.asignaDeducible("Error",e.getMessage());
					return m;
				}finally{
					return m;
